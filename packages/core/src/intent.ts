...
export class InvalidPaymentIntentError extends Error {
  public readonly code = "invalid_payment_intent";
...
function nonNegativeAtomicAmount(value: string, field: string): bigint {
...
export function assertValidPaymentIntent(intent: PaymentIntent, nowSeconds?: number): void {
  if (!intent.task_id.trim()) throw new InvalidPaymentIntentError("task_id is required");
  if (!intent.resource.trim()) throw new InvalidPaymentIntentError("resource is required");
  if (!intent.payee.trim()) throw new InvalidPaymentIntentError("payee is required");
...
export function intentMatchesPaymentRequirements(intent: PaymentIntent, requirements: PaymentRequirements, resourceUrl?: string): boolean {
  try {
    assertValidPaymentIntent(intent);
    if (resourceUrl !== undefined && intent.resource !== resourceUrl) return false;
    return (
      intent.payee.toLowerCase() === requirements.payTo.toLowerCase() &&
...
export function assertIntentAllowsRequirements(
  intent: PaymentIntent,
  requirements: PaymentRequirements,
  nowSeconds?: number,
): void {
  assertValidPaymentIntent(intent, nowSeconds);
  if (intent.payee.toLowerCase() !== requirements.payTo.toLowerCase()) throw new InvalidPaymentIntentError("payee mismatch");
  if (intent.asset_network.asset.toLowerCase() !== requirements.asset.toLowerCase()) throw new InvalidPaymentIntentError("asset mismatch");
  if (intent.asset_network.network !== requirements.network) throw new InvalidPaymentIntentError("network mismatch");
